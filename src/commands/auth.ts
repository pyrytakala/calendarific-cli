...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  calendarific-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  calendarific-cli auth show\n  calendarific-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: calendarific-cli auth set <token>");
      return;
    }
